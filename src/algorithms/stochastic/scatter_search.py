...
def take_step(min_max, current, step_size):
    ...
    for i in xrange(0, len(position)):
        min_v = max(min_max[i][0], current[i] - step_size)
        max_v = min(min_max[i][1], current[i] + step_size)
        position[i] = rand_in_bounds(min_v, max_v)
    return position


def local_search(best, bounds, max_no_improve, step_size):
    count = 0
...
def construct_initial_set(bounds, set_size, max_no_improve, step_size):
    diverse_set = []
    while len(diverse_set) < set_size:
        candidate = {'vector': random_vector(bounds)}
        candidate['cost'] = objective_function(candidate['vector'])
        candidate = local_search(candidate, bounds, max_no_improve, step_size)
        if not any(x['vector'] == candidate['vector'] for x in diverse_set):
...
def recombine(subset, min_max):
    ...
    for p in subset:
        ...
        child = {'vector': [0] * len(min_max)}
        for i in xrange(0, len(child['vector'])):
            child['vector'][i] = p['vector'][i] + (direction * r * d[i])
            if child['vector'][i] < min_max[i][0]:
                child['vector'][i] = min_max[i][0]
            if child['vector'][i] > min_max[i][1]:
                child['vector'][i] = min_max[i][1]
        child['cost'] = objective_function(child['vector'])
        children.append(child)
    return children


def explore_subsets(bounds, ref_set, max_no_improve, step_size):
    was_change = False
    ...
    for subset in subsets:
        ...
        improved = [local_search(candidates[i], bounds, max_no_improve, step_size) for i in xrange(0, len(candidates))]
        for c in improved:
            if not [x for x in ref_set if x['vector'] == c['vector']]:
                c['new'] = True
                ref_set.sort(key=lambda v: v['cost'])
...
def search(bounds, max_iteration, ref_set_size, div_set_size, max_no_improve, step_size, max_elite):
    diverse_set = construct_initial_set(bounds, div_set_size, max_no_improve, step_size)
    ref_set, best = diversify(diverse_set, max_elite, ref_set_size)
    for c in ref_set:
        c['new'] = True
    for iter in xrange(0, max_iteration):
        was_change = explore_subsets(bounds, ref_set, max_no_improve, step_size)
...
def main():
    ...
    best = search(bounds, max_iteration, ref_set_size, diverse_set_size, max_no_improve, step_size, no_elite)
